...
def rasterize_depth(Vw, F, view, screen_px_fn, res=CANVAS):
    """Rasterisation z-buffer barycentrique.
    screen_px_fn(P (n,3)) -> (px (n,), py (n,)) : projection écran des sommets.
    Retourne zbuf (res,res) : profondeur le long de l'axe de vue au pixel le
    plus proche (np.inf si non couvert)."""
    Fw = Vw[F]
