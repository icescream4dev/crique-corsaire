...
// Palette eau 4 couleurs (LucasArts style)
const WP = [0x1a5276, 0x2471a3, 0x2e86c1, 0xd4e6f1];

/** Génère les frames d'eau par palette cycling */
function makeWaterFrames(w:number,h:number):Texture[]{
  // 1. Générer le motif de base (valeurs 0-3 pour les 4 couleurs)
  const base=document.createElement('canvas');base.width=w;base.height=h;
  const bctx=base.getContext('2d')!;
  const bimg=bctx.createImageData(w,h);
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
    const n=(Math.sin(x*0.35+y*0.28)*0.5+Math.sin(x*0.18-y*0.22)*0.3+Math.sin(x*0.55+y*0.45)*0.2)*0.5+0.5;
    const v=Math.floor(n*4); // 0..3
    const i=(y*w+x)*4;
    bimg.data[i]=v;bimg.data[i+1]=0;bimg.data[i+2]=0;bimg.data[i+3]=255; // stocker l'index dans R
  }
  bctx.putImageData(bimg,0,0);
  const baseData=bctx.getImageData(0,0,w,h);

  // 2. Générer 4 frames par rotation de la palette
  ...
  for(let f=0;f<4;f++){
    ...
    for(let i=0;i<w*h;i++){
      const idx=baseData.data[i*4]; // valeur 0-3
      const color=WP[(idx+f)%4];
      const di=i*4;
      img.data[di]=(color>>16)&0xff;
      img.data[di+1]=(color>>8)&0xff;
      img.data[di+2]=color&0xff;
      img.data[di+3]=255;
    }
    ctx.putImageData(img,0,0);
    frames.push(Texture.from(c));
  }
  return frames;
}
...
export class PixiRenderer implements IRenderer {
  ...
  async init(ct:HTMLElement):Promise<void>{
    ...
    this.waterFrames=makeWaterFrames(128,128);
    ...
    this.waterShallow=new TilingSprite({texture:this.waterFrames[2],width:0,height:0});
    this.waterShallow.alpha=0.45;
  ...
  update(_dt:number){
    ...
    this.frame++;
    // Palette cycling : 4 frames, ~4 FPS
    const wf=Math.floor(this.frame/15)%4;
    this.waterDeep.texture=this.waterFrames[wf];
    this.waterShallow.texture=this.waterFrames[(wf+2)%4];
    // Défilement lent (pixel snap via Math.floor implicite de tilePosition)
    this.waterDeep.tilePosition.x=Math.floor(this.frame*0.08);
    this.waterShallow.tilePosition.x=Math.floor(this.frame*0.13);
    this.waterShallow.tilePosition.y=Math.floor(this.frame*0.04);
  }
