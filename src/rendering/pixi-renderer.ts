...
export class PixiRenderer implements IRenderer {
  ...
  private offX = 0; private offY = 0; // canvas top-left in viewport
  ...
  private cv(x: number): number { return x - this.offX; }
  private cy(y: number): number { return y - this.offY; }

  private clamp(): void {
    const sw = this.app.screen.width, sh = this.app.screen.height;
    const iw = this.wW * TILE_SIZE * this.zoom, ih = this.wH * TILE_SIZE * this.zoom;
    // L'île doit toujours chevaucher l'écran d'au moins 30px
    const min = 30;
    this.camX = Math.max(min - iw, Math.min(sw - min, this.camX));
    this.camY = Math.max(min - ih, Math.min(sh - min, this.camY));
  }
  ...
  centerOnWorld(w: number, h: number): void { this.wW = w; this.wH = h; const ww = w * TILE_SIZE, wh = h * TILE_SIZE; this.zoom = Math.min(1, this.app.screen.width / ww, this.app.screen.height / wh); this.camX = this.app.screen.width / 2 - (ww / 2) * this.zoom; this.camY = this.app.screen.height / 2 - (wh / 2) * this.zoom; }
  ...
  onResize(): void { const r = this.app.canvas.getBoundingClientRect(); this.offX = r.left; this.offY = r.top; this.update(0); }
