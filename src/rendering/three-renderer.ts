...
export class ThreeRenderer implements IRenderer {
  ...
  private buildWater(w: number, h: number): void {
    ...
    const mat = new THREE.ShaderMaterial({
      uniforms: {
        ...
        cloudScale: { value: 0.1 },                        // échelle (nuages très grands, peu nombreux)
  ...
  private buildCloudShadow(w: number, h: number): void {
    ...
    const mat = new THREE.ShaderMaterial({
      uniforms: {
        cloudScale: { value: 0.1 },                          // identique au water shader
  ...
  private buildClouds(w: number, h: number): void {
    ...
    const mat = new THREE.ShaderMaterial({
      uniforms: {
        cloudScale: { value: 0.1 },   // identique reflet/ombre
        cloudSpeed: { value: 0.2 },
        time: { value: 0 },
      },
      vertexShader: /* glsl */ `
